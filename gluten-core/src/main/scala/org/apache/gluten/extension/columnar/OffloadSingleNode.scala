...
case class OffloadProjectPartial() extends OffloadSingleNode with LogLevelUtil {
  override def offload(plan: SparkPlan): SparkPlan = plan match {
    case p: ProjectExec if FallbackTags.nonEmpty(p) =>
      val original = p
...
case class OffloadProjectColumnar() extends OffloadSingleNode with LogLevelUtil {
  override def offload(plan: SparkPlan): SparkPlan = plan match {
    case p: ProjectExec if FallbackTags.nonEmpty(p) =>
      val projectColumnar = BackendsApiManager.getSparkPlanExecApiInstance
        .genProjectColumnarExec(p)
      if (projectColumnar != null) {
        if (projectColumnar.doValidate().ok()) {
          projectColumnar
        } else p
      } else p
    case other => other
